...
**Architecture:** 以正传卷章证据索引作为单一事实入口，使用规范姓名/译名映射和阶段/战力数据渲染 TXT；生成后运行结构、篇幅、证据和阶位校验，并对代表性角色做人工抽样。保留现有文件名和单一顶层人物标签。
...
- [x] 先写失败测试，要求一份档案包含身份锚点、欲望/恐惧/底线、条件化场景反应、知识边界、能力限制、口吻示例、阶段切换和证据字段。
- [x] 运行测试确认现有短模板失败。
- [x] 实现分层内容模型：原文事实、阶段解释、扮演规则、口吻示例、存疑项分开存储。
- [x] 对资料薄弱角色强制使用保守规则和禁止施工段落；不从战力表推导性格。
...
- [x] 为每个样本写入至少 2,000 字正文，并提供阶段索引与可回查卷章行为锚点。
...
- [x] 主要人物达到 2,000–3,000 字；证据薄弱人物达到安全的详细规则篇幅，但所有未知内容明确标注。
- [x] 每个阶段单独记录形态、条件和阶位；不把历史回溯、轮回分支和试炼幻境当作主线记忆。
- [x] 更新 README，区分原文事实、总结索引、战力参考个人观点和推定。
...
- [x] 校验 153 份文件的 UTF-8、标签、字段、字符数、重复模板、阶位、阶段和证据类型。
