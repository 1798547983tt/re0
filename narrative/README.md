...
正则产物嵌入代码与资产清单，但不内嵌约 9.65 MB 的图片。当前 `releaseUrl` 留空，因此本地预览能显示图片，单独导入 SillyTavern 时会在图片路径不可用的环境中安全降级。正式使用远程图片前，必须获得单独发布授权，将同一批哈希锁定的文件放到固定版本 HTTPS 地址，并同步更新清单与打包测试；不得使用浮动 `main` 地址。
