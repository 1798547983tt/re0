...
`package-json` 的输入源是 `variable-update/pending.html`、`variable-update/complete.html`、`variable-update/styles.css` 与 `variable-update/assets/manifest.json`，以及未来 packager 源码所定义的固定配置。普通模式只创建或替换以下两个输出文件，不触碰其他 `dist/` 文件：
