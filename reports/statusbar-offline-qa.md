...
离线预览范围内通过。状态栏在宽屏与 320px 窄屏均无页面级横向溢出，九个分区、三态主题控制、严格单组手风琴、按聊天折叠记忆、3＋5 分批列表、人物抽屉、头像编辑、轮回敏感信息折叠、异常数据降级与恶意文本惰性渲染均符合约定。最新预览来源的浏览器控制台未记录 warning 或 error。

本轮复验的 1180 × 900 宽屏中，收起状态栏高 175px，概览核心组展开为 842px，切换到“目标与警示”组为 725px；模式工具条位于分区标题右上方，头部不再保留大块模式面板。320 × 900 新聊天展开详情后，所有信息组默认收起，状态栏高 653px；九个分区保持 59px 高的单行横向滑动标签，页面横向溢出为 0。
