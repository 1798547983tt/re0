...
旧产物有两个结构性风险：展示正则会在编辑路径再次运行，以及替换串只有空的 `<main>`，脚本未执行时自然只剩主题按钮。新产物照《龙族》正式版改成“静态阅读壳层 + 末尾隐藏载体 + 脚本水合”：标题卡、正文容器和安全后备先存在；`runOnEdit=false`，已生成挂载点直接跳过；脚本异常时保留壳层并回退为不可执行文本。`<textarea>` 终止标签仍会被匹配器拒绝，避免载体逃逸。
